...
import { FindVisitByIdDTOInput, FindVisitByIdDTOOutput } from "../../DTOs/visitForm/FindVisitByIdDTO";
import { UseCase } from "../../UseCase";

export class FindVisitByIdUseCase implements UseCase<FindVisitByIdDTOInput, FindVisitByIdDTOOutput>{
    constructor(private readonly visitRepo:VisitFormRepository){}
    async execute(input: FindVisitByIdDTOInput): Promise<FindVisitByIdDTOOutput> {
        try {
            const visitFromPersistence = await this.visitRepo.findById(input.id)

            if(!visitFromPersistence){
                return {
                    success:false,
                    status_code:404,
            ...
            const visit = visitFromPersistence.data
